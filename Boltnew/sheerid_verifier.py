"""SheerID teacher verification main module (Bolt.new)."""
import re
import random
import logging
import httpx
...
class SheerIDVerifier:
    """Teacher identity verifier for the SheerID API (Bolt.new flow)."""
    ...
    def normalize_url(url: str) -> str:
        """Normalize URL (kept for compatibility with existing interfaces)."""
        return url

    @staticmethod
    def parse_verification_id(url: str) -> Optional[str]:
        """Parse verificationId from the URL if present."""
    ...
    def parse_external_user_id(url: str) -> Optional[str]:
        """Parse externalUserId from the URL if present."""
    ...
    def create_verification(self) -> str:
        """Create a new verificationId from the installPageUrl."""
        body = {
        ...
        logger.info("✅ Received verificationId: %s", self.verification_id)
        return self.verification_id

    def _sheerid_request(
        self, method: str, url: str, body: Optional[Dict] = None
    ) -> Tuple[Dict, int]:
        """Send a SheerID API request and return (data, status)."""
        headers = {
    ...
    def verify(
        ...
        """Run the teacher verification flow end-to-end."""
        try:
            ...
            # Step 4: request upload URLs and upload documents
            ...
            if len(documents) != len(assets):
                raise Exception("Number of upload tasks does not match number of files")
            ...
            # Step 5: fetch final status (including rewardCode)
